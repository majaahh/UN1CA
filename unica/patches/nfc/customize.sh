...
elif [ -f "$FW_DIR/$TARGET_FIRMWARE_PATH/system/system/lib/libnfc_nci_jni.so" ] || \
        [ -f "$WORK_DIR/vendor/lib/nfc_nci_nxpsn.so" ] || \
        [ -f "$WORK_DIR/vendor/lib64/nfc_nci_nxpsn.so" ]; then
...
elif [ -f "$FW_DIR/$TARGET_FIRMWARE_PATH/system/system/lib64/libnfc_nci_jni.so" ] || \
        [ -f "$WORK_DIR/vendor/lib64/nfc_nci_nxpsn.so" ]; then
...
elif [ -f "$FW_DIR/$TARGET_FIRMWARE_PATH/system/system/lib64/libnfc_sec_jni.so" ] || \
          [ -f "$WORK_DIR/vendor/lib64/nfc_nci_sec.so" ]; then
    if [ "$TARGET_PLATFORM_SDK_VERSION" -ge "36" ]; then
