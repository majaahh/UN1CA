...
PIF_VERSION="$(curl -sL "https://github.com/UN1CA/static_resources/raw/refs/heads/sixteen/pif/pif.json" | jq -r '[.VERSION] | @tsv')"

if [[ -z "$PIF_VERSION" ]]; then
    LOGE "Failed to fetch pif.json"
    return 1
fi

SMALI_PATCH "system" "system/priv-app/SecSettings/SecSettings.apk" \
    "smali_classes4/io/mesalabs/unica/settings/pif/PIFUtils.smali" "replace" \
    'getFormattedPIFVersion()Ljava/lang/CharSequence;' \
    'CONFIG_PIF_VERSION' \
    "$PIF_VERSION" \
    > /dev/null
SMALI_PATCH "system" "system/priv-app/SecSettings/SecSettings.apk" \
    "smali_classes4/io/mesalabs/unica/settings/pif/PIFUtils.smali" "replace" \
    'lambda$updatePIF$0(Lorg/json/JSONObject;Landroid/content/Context;Landroidx/preference/Preference;)V' \
    'CONFIG_PIF_VERSION' \
    "$PIF_VERSION" \
    > /dev/null
